...
import { searchProducts } from '$lib/server/vendure/searchProducts.graphql';

export const load: PageServerLoad = async function ({ url }) {
  const q: string | null = url.searchParams?.get('q');
  const hits: SearchResult[] | null = q
    ? await searchProducts({ term: q, take: 10, skip: 0, groupByProduct: true })
    : null;
  return {
    hits,
  };
