...
export class MedusaClient {
  ...
  private logLevel: 'verbose' | 'limited' | 'silent' = dev
    ? 'limited'
    : 'silent';
  ...
  async login(
    ...
    return await this.parseAuthCookie(
  ...
  async register(locals: App.Locals, cookies: Cookies, user: User) {
    ...
    return await this.query({
      ...
      .then((res: any) => {
        if (res.ok) {
          return this.login(locals, cookies, email, password)
            .then(() => true)
            .catch(() => false);
        } else return false;
      })
  ...
  async getCollection(handle: string, cacheOptions?: CacheOptions) {
    ...
    return await this.query({
      path: `/store/collections?handle[]=${handle}`,
      ...cacheOptions,
    })
      .then((res: any) => res.json())
      .then((data: any) => data.collections[0])
      .catch(() => null);
  }
  ...
  async getProduct(
    ...
    let product = await this.query({
      path: `/store/products?handle=${handle}`,
      ...cacheOptions,
    })
      .then((res: any) => res.json())
      .then((data: any) => data.products[0])
      .catch(() => null);
    if (!product) {
      return null;
    }
  ...
  async getShippingOptions(locals: App.Locals) {
    ...
    return await this.query({
      locals,
      path: `/store/shipping-options/${locals.cartid}`,
    })
      .then((res: any) => res.json())
      .then((data: any) => data.shipping_options)
      .catch(() => null);
  }
