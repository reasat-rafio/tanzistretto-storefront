<script lang="ts">
  ...
  import { navNotScrolledAnim, navScrolledAnim } from "./animation";
  ...
  $: if (browser) scrolled ? anime(navScrolledAnim) : anime(navNotScrolledAnim);
</script>
...
>
  <div class="container | flex | mx-auto">
    ...
    <ul class="flex-1 flex space-x-5 justify-center">
      {#each menu as { _key, title, pageUrl } (_key)}
        <li
          class="font-title  text-primary flex items-center justify-center text-lg uppercase"
        >
          <a href={pageUrl}>
            {title}
          </a>
    ...
    <div class="flex space-x-5 justify-end">
      ...
      <button
        ...
        class="font-title  text-primary flex items-center justify-center text-lg uppercase relative"
      >
