<script lang="ts">
	...
	const showQuickSearchAction = () =>
		($showQuickSearchDropdown = true)

	$: if (browser)
		scrolled
			? anime(navScrolledAnim)
			: anime(navNotScrolledAnim)
</script>
...
<style lang="postcss">
	...
	.navitem {
		@apply font-title  text-primary flex items-center justify-center text-lg uppercase;
	}
</style>
