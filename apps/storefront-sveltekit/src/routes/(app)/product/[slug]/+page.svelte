      ...
      <div class="mt-6">
        {#each product.variants as variant}
          {#if variant.id === selectedVariantId}
            <button
              type="button"
              class="mb-2 mr-2 whitespace-nowrap rounded-lg border-4 border-lime-600 bg-white px-3 py-2 text-sm font-medium uppercase text-gray-700 hover:bg-white">
              {variant.name}
            </button>
          {:else}
            <button
              ...
              class="mb-2 mr-2 whitespace-nowrap rounded-lg border border-gray-400 bg-white px-3 py-2 text-sm font-medium uppercase text-gray-700 hover:bg-stone-200">
              {variant.name}
            </button>
          {/if}
        {/each}
  ...
  <div class="max-w-screen-lg lg:col-span-2">
    <div class="flex" aria-orientation="horizontal" role="tablist">
      <button
        ...
        class={tab === 'reviews'
          ? 'mr-4 whitespace-nowrap border-b-2 border-lime-600 p-3 pr-4 font-medium text-gray-800'
      ...
      <button
        ...
        on:click={() => (tab = 'faq')}
        class={tab === 'faq'
          ? 'mr-4 whitespace-nowrap border-b-2 border-lime-600 p-3 px-4 font-medium text-gray-800'
    ...
    <!-- {#if tab == 'reviews'}
      <ProductReviews
        bind:reviewForm={data.reviewForm}
        {product}
        {user}
        {reviews} />
    {:else if tab == 'faq'}
