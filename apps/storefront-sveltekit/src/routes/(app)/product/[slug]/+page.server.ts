...
import { getProduct } from '$lib/server/vendure/getProduct.graphql';
import { addReviewReq } from '$lib/utils/validators';
...
export const actions: Actions = {
  addReview: async ({ request, locals }) => {
    const form = await superValidate(request, zod(addReviewReq), {
      id: 'review',
    });

    if (!locals.user)
      return message(form, 'You must be signed in to add a review', {
        status: 401,
      });
